...
Window {
    ...
    //değerlere göre renk döndüren yardımcı fonksiyonlar
    ...
    function getPulseColor(val) {
        ...
        if (val >= 60 && val <= 100) return "#0ea5e9"; //mavi-normal nabız
        if (val > 100 || (val > 0 && val < 60)) return "#f59e0b"; // turuncu-kritik nabız
        return "#ef4444";
    }

    // hem SpO2 hem de Pulse için ortak durum metni
    function getStatusText(val,type){
        if (val === 0) return "Place Your Fİnger On the Sensor";
        if(type === "spo2" ) {
    ...
    Row {
        ...
        spacing: 40 // iki kutu arası boşluk

        // 1.Kutu- Saturasyon -solda
        Rectangle {
            width: 320
            height: 320
            color: "#1e293b"
            ...
            // metinlerin alt alta ortaladığı taşıyıcı
            Column {
                anchors.centerIn: parent
                spacing: 20
        ...
        Rectangle {
            ...
            //metinlerin alt alta ortalandığı taşıyıcı
            Column {
                anchors.centerIn: parent
                spacing: 20
